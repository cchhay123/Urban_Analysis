...
```{r}
weather <- weather %>% 
  group_by(month(weather$time), day(weather$time)) %>% 
  summarize

table(month(weather$times))
...
```{r}
icon_count <- weather %>% group_by(icon = icon,
                                   summary = summary) %>% summarize(count = n())
...
weather$weather[weather$weather %in% c("partly-cloudy-day", "partly-cloudy-night")] <- "cloudy"
...
ggplot(weather_n_trip, aes(weather, avg_hourly_rental)) + geom_bar(stat = "identity")
```
...
```{r}
ggplot(weather_n_trip) + geom_boxplot(aes("", avg_hourly_rental)) + facet_wrap(~ weather)

ggplot(bike) + geom_boxplot(aes("", duration_m)) + facet_wrap(~ weather)
...
data_agg <- bike %>% 
  group_by(hour = hour(starttime_hour), weather, weekend) %>%  
  summarise(
    num_trip = n(),
    avg_duration = mean(duration_m)
  )

data_agg %>% ggplot(aes(hour, num_trip, color = weather, group = weather)) + geom_line() + facet_wrap(~weekend)
...
temp_hourly <- bike %>%  
  group_by(temp_group) %>% 
  summarise(avg_hourly = n() / n_distinct(hour(starttime_hour)), groups = "drop")

ggplot(temp_hourly, aes(x=temp_group, y=avg_hourly)) +
...
ggplot(bike, aes(temperature, duration_m)) + geom_point(alpha = .5, color = 'blue') + geom_smooth(method = 'lm', color = 'red', se = TRUE)
